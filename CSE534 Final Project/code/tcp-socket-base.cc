...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddTraceSource ("Rx",
                     ...
                     "ns3::TcpSocketBase::TcpTxRxTracedCallback")
    .AddTraceSource ("RWND Trace",
                     "Remote side's flow control window",
                     MakeTraceSourceAccessor (&TcpSocketBase::m_rWndTrace),
                     "ns3::TracedValueCallback::Uint32")
  ;
  return tid;
}
...
TcpSocketState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketState")
    ...
    .AddTraceSource("ReceiveWindowSize",
                    ...
                    MakeTraceSourceAccessor(&TcpSocketState::m_rcvWnd),
                    "ns3::TracedValue::Uint32Callback")
  ;
  return tid;
}
    ...
    m_isFirstPartialAck (true)
{
  ...
  NS_ASSERT (ok == true);
  ok = m_tcb->TraceConnectWithoutContext ("ReceiveWindowSize",
                                          MakeCallback (&TcpSocketBase::UpdateRcvWindow, this));
}
    ...
    m_rxTrace (sock.m_rxTrace)
{
  ...
  ok = m_tcb->TraceConnectWithoutContext ("HighestSequence",
                                          MakeCallback (&TcpSocketBase::UpdateHighTxMark, this));
  NS_ASSERT (ok == true);
  ok = m_tcb->TraceConnectWithoutContext ("Receive Window Size",
                                          MakeCallback (&TcpSocketBase::UpdateRcvWindow, this));
}
...
//newly add
void
TcpSocketBase::UpdateRcvWindow(uint32_t oldValue, uint32_t newValue)
{
  m_rWndTrace(oldValue, newValue);
}
